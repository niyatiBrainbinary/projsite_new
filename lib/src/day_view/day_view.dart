...
class DayViewState<T extends Object?> extends State<DayView<T>> {
  ...
  // late PageController chartPageController;
  ...
  void initState() {
    ...
    chartPageController = PageController(initialPage: _currentIndex);
    _eventArranger = widget.eventArranger ?? SideEventArranger<T>();
    _assignBuilders();
    matchIndex=_currentIndex;
  ...
  void didUpdateWidget(DayView<T> oldWidget) {
    ...
    if (widget.minDay != oldWidget.minDay ||
      ...
      chartPageController.jumpToPage(_currentIndex);
    }
  ...
  // void getDate1() {
  //   final date = DateTime.parse(currentDate.toString());
  //   var showDate = DateFormat.yMMMMd().format(date);
  //
  //
  //   print('Date: $date');
  //   print('Start of week: ${getDate(date.subtract(Duration(days: date.weekday - 1)))}');
  //   print('Start of 1: ${getDate(date.subtract(Duration(days: date.weekday - 2)))}');
 ...
 // DateTime getDate(DateTime d) => DateTime(d.year, d.month, d.day);
  ...
  _dayView(){
    ...
    if(calenderCub.status==true){
      _currentDate=calenderCub.projectDate;
    } else{
      calenderCub.projectDate=_currentDate;
    }
    calenderCub.fun();
  ...
  void _onPageChange(int index) {
    final calenderCub =BlocProvider.of<CalendarCubit>(context);
    BlocProvider.of<CalendarCubit>(context).status=false;
    if (mounted) {
      setState(() {
    ...
    if(isSwipe==false){
      ...
      DateTime _projectDate= DateTime.parse("${DateFormat('yyyy-MM-dd').format(DateTime.parse("${calenderCub.projectDate}"))} 00:00:00.000");
        int a = _currentDate.compareTo(_projectDate) == 1
            ? _currentDate.difference(_projectDate).inDays
            : -_projectDate.difference(_currentDate).inDays;

        matchIndex=matchIndex+a;
        print("swipe_matchIndex=${matchIndex}");
        calenderCub.projectDate = _currentDate;

    }
    widget.onPageChange?.call(_currentDate, matchIndex);

    String currentPageDate=DateFormat('yyyy-MM-dd').format(DateTime.parse("${_currentDate}"));
    String lastDate=DateFormat('yyyy-MM-dd').format(DateTime.parse("${matchDate.add(Duration(days: 7))}"));
    String firstDate=DateFormat('yyyy-MM-dd').format(DateTime.parse("${matchDate.subtract(Duration(days: 1))}"));

    if(lastDate==currentPageDate){
      datePageController.nextPage(duration: Duration(milliseconds: 1),
          curve: Curves.easeIn);
    } else if(firstDate==currentPageDate){
      datePageController.previousPage(duration: Duration(milliseconds: 1),
          curve: Curves.easeIn);
    }
    isSwipe=false;
    setState(() {});
  ...
  ///
  ///
  void nextPage({Duration? duration, Curve? curve}) => chartPageController.nextPage(
  ...
  void previousPage({Duration? duration, Curve? curve}) =>
      chartPageController.previousPage(
        duration: duration ?? widget.pageTransitionDuration,
        curve: curve ?? widget.pageTransitionCurve,
      );
  ...
  ///
  ///
  void jumpToPage(int page) => chartPageController.jumpToPage(page);
  ...
  Future<void> animateToPage(int page, {Duration? duration, Curve? curve}) =>
      chartPageController.animateToPage(page,
          duration: duration ?? widget.pageTransitionDuration,
          curve: curve ?? widget.pageTransitionCurve);
  ...
  void jumpToDate(DateTime date) {
    ...
    chartPageController.jumpToPage(_minDate.getDayDifference(date));
  }
  ...
  Future<void> animateToDate(DateTime date,
    ...
    await chartPageController.animateToPage(
